<?php
/**
 * Plugin Name: Ocasio Admin Username Changer
 * Plugin URI:  https://kevinocasio.com/wordpress-plugins/ocasio-admin-username-changer/
 ...
 * Text Domain: ocasio-admin-username-changer
 */
...
define('OCASIO_AUC_VERSION', '1.0.0');
define('OCASIO_AUC_PLUGIN_DIR', plugin_dir_path(__FILE__));
define('OCASIO_AUC_PLUGIN_URL', plugin_dir_url(__FILE__));

/**
 * Helper: Smart Brand URL Resolver (Keeps links local on .local, points to live domain on production)
 */
function ocasio_auc_brand_url($path = '/wordpress-plugins/') {
...
/**
 * 1. Admin Menu Placement under Ocasio Plugins (Position 65)
 */
function ocasio_auc_admin_menu() {
	...
	add_submenu_page(
		...
		'manage_options',
		'ocasio-admin-username-changer',
		'ocasio_auc_render_page'
	);
}
add_action('admin_menu', 'ocasio_auc_admin_menu');

/**
 * 2. Action Link on Plugins Screen
 */
function ocasio_auc_settings_action_link($links) {
	$settings_url = admin_url('admin.php?page=ocasio-admin-username-changer');
	$settings_link = '<a href="' . esc_url($settings_url) . '">' . esc_html__('Change Username', 'ocasio-admin-username-changer') . '</a>';
	array_unshift($links, $settings_link);
	return $links;
}
add_filter('plugin_action_links_' . plugin_basename(__FILE__), 'ocasio_auc_settings_action_link');

/**
 * 3. Conditional Admin Asset Enqueuing
 */
function ocasio_auc_enqueue_admin_assets($hook) {
	...
	if (strpos($hook, 'ocasio-admin-username-changer') !== false || strpos($hook, 'ocasio-plugins-main') !== false || $hook === 'toplevel_page_ocasio-plugins-main') {
		wp_enqueue_style('ocasio-auc-admin-css', plugins_url('assets/admin.css', __FILE__), array(), OCASIO_AUC_VERSION);
		wp_enqueue_script('ocasio-auc-admin-js', plugins_url('assets/admin.js', __FILE__), array('jquery'), OCASIO_AUC_VERSION, true);
		wp_localize_script('ocasio-auc-admin-js', 'ocasio_vars', array(
			'ajaxurl'     => admin_url('admin-ajax.php'),
			'suite_nonce' => wp_create_nonce('ocasio_suite_toggle_nonce'),
		));
		wp_localize_script('ocasio-auc-admin-js', 'ocasio_301_vars', array(
			'ajaxurl'     => admin_url('admin-ajax.php'),
			'suite_nonce' => wp_create_nonce('ocasio_suite_toggle_nonce'),
		));
	}
}
add_action('admin_enqueue_scripts', 'ocasio_auc_enqueue_admin_assets');

/**
 * 4. Core Logic: Safe Database Username Mutation & Session Refresh
 */
function ocasio_auc_handle_submit() {
	if (!isset($_POST['ocasio_auc_submit'])) {
		return;
	}

	check_admin_referer('ocasio_auc_action', 'ocasio_auc_nonce');

	if (!current_user_can('manage_options')) {
		wp_die(esc_html__('You do not have sufficient permissions to perform this action.', 'ocasio-admin-username-changer'));
	}

	$new_username = isset($_POST['ocasio_auc_new_username']) ? sanitize_user(trim($_POST['ocasio_auc_new_username']), true) : '';
	$current_user = wp_get_current_user();
	$old_username = $current_user->user_login;

	// Validation
	if (empty($new_username)) {
		add_settings_error('ocasio_auc_messages', 'ocasio_auc_empty', esc_html__('Username cannot be empty.', 'ocasio-admin-username-changer'), 'error');
		return;
	}

	if ($new_username === $old_username) {
		add_settings_error('ocasio_auc_messages', 'ocasio_auc_same', esc_html__("That's already your current username.", 'ocasio-admin-username-changer'), 'error');
		return;
	}

	if (!validate_username($new_username)) {
		add_settings_error('ocasio_auc_messages', 'ocasio_auc_invalid', esc_html__('Username contains invalid characters.', 'ocasio-admin-username-changer'), 'error');
		return;
	}

	if (username_exists($new_username)) {
		add_settings_error('ocasio_auc_messages', 'ocasio_auc_exists', esc_html__('This username is already taken. Please choose another.', 'ocasio-admin-username-changer'), 'error');
		return;
	}
	...
	if ($result !== false) {
		...
		$redirect_url = add_query_arg(
			array(
				'page'    => 'ocasio-admin-username-changer',
				'updated' => 'true',
			),
			admin_url('admin.php')
		);
		...
		add_settings_error('ocasio_auc_messages', 'ocasio_auc_db_error', esc_html__('Database error. Could not update username.', 'ocasio-admin-username-changer'), 'error');
	}
}
add_action('admin_init', 'ocasio_auc_handle_submit');

/**
 * 5. Dedicated Action Screen HTML (Centered 620px Enclosed Card)
 */
function ocasio_auc_render_page() {
	if (!current_user_can('manage_options')) {
		return;
	}

	$current_user = wp_get_current_user();
	$errors       = get_settings_errors('ocasio_auc_messages');
	$is_updated   = isset($_GET['updated']) && 'true' === $_GET['updated'];

	$hub_url    = ocasio_auc_brand_url('/wordpress-plugins/');
	$author_url = ocasio_auc_brand_url('/');
	?>
		...
		<div class="ko-plugin-card">
			...
			<div class="ko-plugin-header">
				<h1 class="ko-plugin-header-title">
					...
					<span class="ko-title-text">ADMIN USERNAME CHANGER</span>
				</h1>
			</div>

			<!-- Content Stage -->
			<div class="ko-plugin-body">
				<p class="ko-plugin-intro">Safely update your login username to harden security or change your administrator handle without database scripts.</p>

				<?php if (!empty($errors)): ?>
					<?php foreach ($errors as $error): ?>
						<div class="ko-alert-error" style="background:#fef2f2; border:1px solid #fecaca; color:#991b1b; padding:12px 16px; border-radius:6px; margin-bottom:18px; font-size:13px; font-weight:500;">
							<p style="margin:0;"><?php echo esc_html($error['message']); ?></p>
						</div>
					<?php endforeach; ?>
				<?php endif; ?>

				<?php if ($is_updated): ?>
					<div class="ko-alert-success" style="background:#f0fdf4; border:1px solid #bbf7d0; color:#166534; padding:12px 16px; border-radius:6px; margin-bottom:18px; font-size:13px; font-weight:500;">
						<p style="margin:0;"><strong style="color:#15803d;">Success:</strong> Your login username has been updated! Remember to use your new username next time you sign in.</p>
					</div>
				<?php endif; ?>

				<form method="post" action="">
					<?php wp_nonce_field('ocasio_auc_action', 'ocasio_auc_nonce'); ?>
					...
					<div class="ko-setting-box">
						...
						<div class="ko-setting-row">
							...
							<input type="text" value="<?php echo esc_attr($current_user->user_login); ?>" disabled class="ko-input-text" style="width:200px; padding:7px 12px; border-radius:6px; border:1px solid #cbd5e1; background:#f8fafc; font-weight:600; color:#64748b;">
						</div>
						...
						<div class="ko-setting-row">
							...
							<input type="text" name="ocasio_auc_new_username" placeholder="Enter new username..." class="ko-input-text" required autocomplete="off" style="width:200px; padding:7px 12px; border-radius:6px; border:1px solid #cbd5e1; font-weight:600;">
						</div>
					</div>
					...
					<div class="ko-submit-wrap">
						...
						$btn_class = 'ko-btn-submit' . ($is_updated ? ' ko-btn-saved' : '');
						?>
						<button type="submit" name="ocasio_auc_submit" id="ocasio-auc-submit-btn" class="<?php echo esc_attr($btn_class); ?>">
							<?php echo esc_html($btn_text); ?>
						</button>
						<p class="ko-submit-note" style="margin-top:10px; font-size:12px; color:#64748b;">Note: Changing your username updates your login credentials immediately.</p>
					</div>
				</form>
			</div>
...
// -------------------------------------------------------------------------
// 6. MASTER OCASIO PLUGINS SUITE DASHBOARD CALLBACK (17-PLUGIN GRID)
// -------------------------------------------------------------------------
